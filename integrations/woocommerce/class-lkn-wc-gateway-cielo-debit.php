...
final class Lkn_Wc_Gateway_Cielo_Debit extends WC_Payment_Gateway
{
    ...
    public function __construct()
    {
        ...
        if ($post && has_shortcode($post->post_content, 'woocommerce_checkout') && 'yes' === $gateway_enabled['enabled']) {
            wp_enqueue_script('lkn-fix-script', LKN_WC_GATEWAY_CIELO_URL . 'public/js/lkn-dc-script-fix.js', array('wp-i18n', 'jquery'), $this->version, false);
    ...
    public function lkn_admin_load_script(): void
    {
        ...
        if ('wc-settings' === $page && 'checkout' === $tab && $section == $this->id) {
            wp_enqueue_script('lknWCGatewayCieloDebitSettingsLayoutScript', LKN_WC_GATEWAY_CIELO_URL . 'admin/js/lkn-wc-gateway-admin-layout.js', array('jquery'), $this->version, false);
            wp_localize_script('lknWCGatewayCieloDebitSettingsLayoutScript', 'lknWcCieloTranslationsInput', array(
                ...
                'mordernVersion' => LKN_WC_GATEWAY_CIELO_URL . 'public/images/modern-version.png',
                'standardVersion' => LKN_WC_GATEWAY_CIELO_URL . 'public/images/standard-version.png'
            ));
            wp_enqueue_style('lkn-admin-layout', LKN_WC_GATEWAY_CIELO_URL . 'admin/css/lkn-admin-layout.css', array(), $this->version, 'all');
            wp_enqueue_script('lknWCGatewayCieloDebitClearButtonScript', LKN_WC_GATEWAY_CIELO_URL . 'admin/js/lkn-clear-logs-button.js', array('jquery', 'wp-api'), $this->version, false);
    ...
    public function lkn_payment_gateway_scripts(): void
    {
        ...
        if ('production' === $env) {
            wp_enqueue_script('lkn-dc-script', LKN_WC_GATEWAY_CIELO_URL . 'public/js/lkn-dc-script-prd.js', array('wp-i18n', 'jquery', 'wp-api'), $this->version, false);
            wp_set_script_translations('lkn-dc-script', 'lkn-wc-gateway-cielo', LKN_WC_CIELO_TRANSLATION_PATH);
        } else {
            wp_enqueue_script('lkn-dc-script', LKN_WC_GATEWAY_CIELO_URL . 'public/js/lkn-dc-script-sdb.js', array('wp-i18n', 'jquery', 'wp-api'), $this->version, false);
            wp_set_script_translations('lkn-dc-script', 'lkn-wc-gateway-cielo', LKN_WC_CIELO_TRANSLATION_PATH);
        }
        wp_localize_script('lkn-dc-script', 'lknDCDirScript3DSCieloShortCode', LKN_WC_GATEWAY_CIELO_URL . 'public/js/BP.Mpi.3ds20.min.js');
        wp_localize_script('lkn-dc-script', 'lknDCScriptAllowCardIneligible', $this->get_option('allow_card_ineligible', 'no'));
        wp_enqueue_script('lkn-mask-script', LKN_WC_GATEWAY_CIELO_URL . 'public/js/formatter.js', array('jquery'), $this->version, false);
        wp_enqueue_script('lkn-mask-script-load', LKN_WC_GATEWAY_CIELO_URL . 'public/js/define-mask.js', array('lkn-mask-script', 'jquery'), $this->version, false);

        wp_enqueue_script('lkn-cc-dc-installment-script', LKN_WC_GATEWAY_CIELO_URL . 'public/js/lkn-cc-dc-installment.js', array('jquery'), $this->version, false);
        ...
        wp_enqueue_style('lkn-dc-style', LKN_WC_GATEWAY_CIELO_URL . 'admin/css/lkn-dc-style.css', array(), $this->version, 'all');

        wp_enqueue_style('lkn-mask', LKN_WC_GATEWAY_CIELO_URL . 'admin/css/lkn-mask.css', array(), $this->version, 'all');

        wp_enqueue_script('lkn-fix-token-script', LKN_WC_GATEWAY_CIELO_URL . 'public/js/lkn-fix-token-script.js', array('jquery', 'wp-api'), $this->version, false);
    }
    ...
    {
        wp_enqueue_style('lknWCGatewayCieloFixIconsStyle', LKN_WC_GATEWAY_CIELO_URL . 'admin/css/lkn-fix-icons-styles.css', array(), $this->version, 'all');
...
<?php
    ...
    public function validate_fields()
    {
        ...
        if ('no' === $validateCompatMode) {
            ...
            $validdcNumber = $this->lkn_validate_card_number($dcnum, true);
     ...
     * @return bool
     */
    private function lkn_validate_card_number($dcnum, $renderNotice)
    {
        if (empty($dcnum)) {
            if ($renderNotice) {
                $this->lkn_add_notice_once(__('Debit Card number is required!', 'lkn-wc-gateway-cielo'), 'error');
            }
        ...
        if (true !== $isValid || strlen($dcnum) < 12) {
            if ($renderNotice) {
                $this->lkn_add_notice_once(__('Debit Card number is invalid!', 'lkn-wc-gateway-cielo'), 'error');
            }
    ...
    private function lkn_validate_exp_date($expDate, $renderNotice)
    {
        ...
        try {
            ...
            if ($today > $expDate) {
                if ($renderNotice) {
                    $this->lkn_add_notice_once(__('Debit card is expired!', 'lkn-wc-gateway-cielo'), 'error');
                }
