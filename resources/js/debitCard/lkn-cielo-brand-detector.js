/**
 * Cielo Debit Card Brand Detector
 * 
 * Monitors debit card number input and applies visual effects to brand icons.
...
(function() {
    ...
    function initializeBrandDetector() {
        // Get elements - debit specific ID
        cardNumberInput = document.getElementById('lkn_dcno');
        brandIcons = document.querySelectorAll('#cielo-debit-card-brands .card-brand-icon');
        
        // Setup card input listeners apenas se input existir
        if (cardNumberInput && !cardNumberInput.hasAttribute('data-cielo-debit-initialized')) {
            cardNumberInput.setAttribute('data-cielo-debit-initialized', 'true');
    ...
    function initializeSubmitButton() {
        const cieloSubmitBtn = document.getElementById('cielo-debit-submit-btn');
                ...
                if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
                    ...
                    const hasTargetElements = Array.from(mutation.addedNodes).some(node => {
                        if (node.nodeType === Node.ELEMENT_NODE) {
                            return node.querySelector('#lkn_dcno, #cielo-debit-card-brands .card-brand-icon') ||
                                   node.id === 'lkn_dcno' ||
