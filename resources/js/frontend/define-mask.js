window.addEventListener('DOMContentLoaded', function () {
    ...
    if (noLoginCheckout && noLoginCheckout.value === 'true') {
        var ccno = document.getElementById('lkn_ccno');
        if (ccno) {
            // Credit card
            var formattedCc = new Formatter(ccno, {
                'patterns': [
                    // elo
        ...
        var ccExpDate = document.getElementById('lkn_cc_expdate');
        if (ccExpDate) {
            var formattedExpDateCc = new Formatter(ccExpDate, {
                'pattern': '{{99}} / {{99}}'
            });
        }

        var ccCvv = document.getElementById('lkn_cc_cvc');
        if (ccCvv) {
            var formattedCvvCc = new Formatter(ccCvv, {
                'pattern': '{{9999}}'
            });
        }

        var dcno = document.getElementById('lkn_dcno');
        if (dcno) {
            // Debit card
            var formattedDc = new Formatter(dcno, {
                'patterns': [
                    // elo
                    { '(4011|431274|438935|451416|457393|4576|457631|457632|504175|627780|636297|636368|636369|(6503[1-3])|(6500(3[5-9]|4[0-9]|5[0-1]))|(6504(0[5-9]|1[0-9]|2[0-9]|3[0-9]))|(650(48[5-9]|49[0-9]|50[0-9]|51[1-9]|52[0-9]|53[0-7]))|(6505(4[0-9]|5[0-9]|6[0-9]|7[0-9]|8[0-9]|9[0-8]))|(6507(0[0-9]|1[0-8]))|(6507(2[0-7]))|(650(90[1-9]|91[0-9]|920))|(6516(5[2-9]|6[0-9]|7[0-9]))|(6550(0[0-9]|1[1-9]))|(6550(2[1-9]|3[0-9]|4[0-9]|5[0-8]))|(506(699|77[0-8]|7[1-6][0-9))|(509([0-9][0-9][0-9])))': '{{9999}} {{9999}} {{9999}} {{9999}}' },
                    // hipercard
        ...
        var dcExpDate = document.getElementById('lkn_dc_expdate');
        if (dcExpDate) {
            var formattedExpDateDc = new Formatter(dcExpDate, {
                'pattern': '{{99}} / {{99}}'
            });
        }

        var dcCvv = document.getElementById('lkn_dc_cvc');
        if (dcCvv) {
            var formattedCvvDc = new Formatter(dcCvv, {
                'pattern': '{{9999}}'
            });
        }
    } else {
